  ...
  export default function ClientsListClient() {
    ...
    const loadClients = useCallback(
      async (uid: string, showSpinner = true) => {
        ...
        setError(null);
// TODO: add pagination (limit + startAfter)
        try {
  ...
  () => [
    {
      key: "Active",
      label: t.kpi?.active ?? "Active",
      value: kpis.active,
    },
    {
      key: "Prospect",
      label: t.kpi?.prospect ?? "Prospect",
      value: kpis.prospect,
    },
    {
      key: "Inactive",
      label: t.kpi?.inactive ?? "Inactive",
      value: kpis.inactive,
    },
  ],
  ...
  />
    ...
    {chartData.map((entry, index) => {
      ...
      if (entry.key === "Active") color = "#22c55e"; // green
      if (entry.key === "Prospect") color = "#f59e0b"; // amber
      if (entry.key === "Inactive") color = "#ef4444"; // red
    ...
    {filteredClients.map((c) => (
      <div
       ...
       onClick={(e) => {
  if ((e.target as HTMLElement).closest(".client-card-menu-btn")) return;
